...
The ``kite \& dart tiling'',\index{index}{kite \& dart
tiling} pictured in Fig.~\ref{Intro:fig1}, has been widely
...
periodic\index{index}{periodic tiling} structures such as
...
Culik,\index{index}{Kari and Culik tiling} made from copies
...
``tiling''\index{index}{tiling} in place of jigsaw puzzle
...
$X_{\mathcal{A}}$\index{symbols}{$X_{A}$} of \emph{all
...
\emph{local}\index{index}{local} examination, where the
...
rule''\index{index}{substitution rule} associates a large
...
approach\index{symbols}{$m(x, y)$} to physics of Maxwell's
...
``long range order'',\index{index}{long range order} and
...
$T^{t}$\index{symbols}{$T^{t}$} translates it to the
...
\index{symbols}{$|t|$}$|t|=(t_{1}^{2}+t_{2}^{2}+\cdots+t_{d}^{2})^{1/2}$
...
An ``intermediate''\index{index}{intermediate order} case
...
\end{equation}
where $B_{N}$\index{symbols}{$B_{N}$} is a ball, centered
...
``square-like''\index{index}{square-like} tiles as in
...
As a set,
$\mathcal{A}^{\mathbb{Z}^{d}}$\index{symbols}{$\mathcal{A}^{\mathbf{Z}^{d}}$}
...
\mathbb{I}$\index{symbols}{$T^{t} \mathbb{I}$} through
$T^{t} \mathbb{I}(f)=
\mathbb{I}(T^{-t}(f))$\index{symbols}{$\mathbb{I}(f)$}.
...
of the type $\chi_{p}$\index{symbols}{$\chi_{p}$}
...
frequency $\nu(p)$ of the word\index{index}{word} $p$ in
...
compact set containing) its ``orbit''\index{index}{orbit}
$O(\tilde{x})\equiv\{T^{t}\tilde{x}:t\in
\mathbb{Z}^{d}\}$\index{symbols}{$O(x)$}, in order to use
...
Let $\mathcal{W}$\index{symbols}{$\mathcal{W}$} be the set
...
``subword''\index{index}{subword} of the word $q\in
...
are ``(translation) invariant'',\index{index}{translation
invariant} that is, such that $x\in X$ implies $T^{t}x\in
...
``subshift'',\index{index}{subshift} as is standard in
ergodic theory.

The first kind of $X$ we consider are the ``substitution
subshifts''\index{index}{substitution subshifts}. To define
them we assume given a ``substitution
function'',\index{index}{substitution function}
...
$C(S)$\index{symbols}{$C(S)$} of complex continuous
...
We now apply the Perron-Frobenius theorem,\index{index}{Perron-Frobenius theorem} in a slightly unusual form \cite[p. 136]{bib:Rue} (see Appendix \ref{appIII:appIII} for notation):
...
The spectral radius\index{index}{spectral radius} of $\tilde{M}$ (see Appendix \ref{appIII:appIII}) \emph{is} $\lambda$.
\end{corollary}
...
``quasicrystals''\index{index}{quasicrystals} which are
...
symmetry\index{index}{approximate 5-fold symmetry}.
...
10-\emph{fold} symmetry\index{index}{10-fold symmetry} of
...
tiling\index{index}{quaquaversal tiling} is shown in
Fig.~\ref{ch04:fig32}.
\begin{figure}[h]
